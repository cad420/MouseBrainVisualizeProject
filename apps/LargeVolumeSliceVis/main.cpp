...
void RunRenderLoop(bool async){
    ...
    if(async){
        slice_render = [&]()->const Image&{//如果不显示指定返回类型 lambda的函数返回类型会是Image 因为function = 不要求返回类型一定相同 它会去引用
            SliceExt sliceExt{slice,SliceHelper::GetSliceLod(slice),volume.getVoxel()*SliceHelper::SliceStepVoxelRatio,0.f};
      ...
      while(SDL_PollEvent(&event)){
          ...
          case SDL_MOUSEMOTION:{
              if (event.motion.state & SDL_BUTTON_RMASK)
              {
                  ...
                  slice.origin +=
                      - slice.x_dir * x_offset * slice.voxels_per_pixel * voxel
...
void RunDivideAndMergeRenderLoop(bool async){
    ...
    if(async){
        ...
        slice_render = [&]()->const Image&{
            ...
            auto slice_render_task = [&](const SliceExt& sub_slice,SliceRenderer* slice_renderer,GPUResource* gpu_resource)->const Image&{
                FrustumExt view_frustum{};
            ...
            for(int i = 0; i < gpu_count; i++){
                sub_images.emplace_back(std::async(std::launch::async,slice_render_task,sub_slices[i],slice_renderers[i],gpu_resources[i].get()));
            }
